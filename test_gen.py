...
parser.add_argument('--ckpt', type=str, default='./logs_gen/GEN_2022_04_13__13_54_42/sde_chair.pt')
parser.add_argument('--categories', type=str_list, default=['chair'])
parser.add_argument('--save_dir', type=str, default='./results')
parser.add_argument('--device', type=str, default='cuda:0')
# Datasets and loaders
parser.add_argument('--dataset_path', type=str, default='/data2/tli/sp-diffusion-point-cloud/data/shapenet.hdf5')
...
# Logging
save_dir = os.path.join(args.save_dir, 'GEN_SDE_%s_%d' % ('_'.join(args.categories), int(time.time())) )
...
# test_dset = part_dataset_all_normal.PartNormalDataset(root=args.dataset_path, npoints=2048, split='test')
# test_loader = DataLoader(test_dset, batch_size=args.batch_size, num_workers=args.num_workers)
...
gen_pcs = []
all_code = []
for i in tqdm(range(2), 'Generate'):
...
for i in range(2):
    pcu.save_mesh_v(os.path.join(save_dir,f'out_{i}.ply'), gen_pcs[i].detach().cpu().numpy())

code_delta = (all_code[0]-all_code[1])/10
print('code, ',len(all_code), all_code[0].shape, code_delta.shape)
all_recons_delta = []
for i in range(1,11,1):
    with torch.no_grad():
        code = (all_code[0] + code_delta*i).to(args.device)
        recons = model.sample(code, args.sample_num_points, flexibility=ckpt['args'].flexibility)
    all_recons_delta.append(recons.detach().cpu())
for i in range(10):
    pcu.save_mesh_v(os.path.join(save_dir,f'rec_del{i}.ply'), all_recons_delta[i].numpy())
